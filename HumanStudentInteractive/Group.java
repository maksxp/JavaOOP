...
private Student [] sortBySex (Student [] group){
	//option without comparator and lambda function
...
//	}
//	return sortedBySex;
	// option uses Comparator and lambda function
	Arrays.sort(group, (nOne, nTwo)->{
		if((nOne.getSex()==SexStudent.MALE)&&(nTwo.getSex()==SexStudent.FEMALE))
	return -1;
		else if ((nOne.getSex()==SexStudent.FEMALE)&&(nTwo.getSex()==SexStudent.MALE))	
			return 1;
		else 
			return 0;
	});
...
//return temp;
//	option uses Comparator and lambda function
Arrays.sort(group, (nOne, nTwo)-> nOne.getAge()-nTwo.getAge());
return group;
	}
...
//return temp;
//option uses Comparator and lambda function
Arrays.sort(group, (nOne, nTwo)-> nOne.getAverageGrade()-nTwo.getAverageGrade());
return group;
	}
@Override
...
//method to save group into file
public static void groupToFile (Group group, File fileToSave) throws IOException {
	if (group==null||fileToSave==null)
		System.out.println("null pointer exception");
			fileToSave.createNewFile();
	
	try(PrintWriter writer = new PrintWriter(fileToSave.getAbsolutePath())){
	writer.write(Arrays.toString(group.group));
}
	catch (IOException e) {
		System.out.println(e+"something's gone wrong ");
	}
}
public static Group fileToGroup (AddableToGroup file){
	return new Group(file.addToGroupFromFile(file.getSource()));
}
}
